...
async def search_albums(
    ...
    """
    # Используем multi_entity_search для поиска альбомов
    # Поскольку multi_entity_search возвращает все типы сущностей, мы извлекаем только альбомы
    results = await search_service.multi_entity_search(query=query, limit=limit)
    albums_found = results.get("albums", [])
    
    # ResponseModel - List[Album], поэтому просто возвращаем список альбомов
    return albums_found
...
async def update_album(
    ...
    # Обновляем альбом в Elasticsearch
    if updated_album:
        # Получаем имя артиста для индексации
...
async def delete_album(
    ...
    # Удаляем альбом из Elasticsearch
